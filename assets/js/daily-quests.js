// Sistema de Missões Diárias com Timer 24h
...
function isQuestAvailable(category) {
    const lastCompleted = localStorage.getItem(`quest_${category}_completed`);
...
function finishQuiz() {
    ...
    // Marcar como completado e iniciar timer
    const now = new Date().getTime();
    localStorage.setItem(`quest_${currentCategory}_completed`, now.toString());
    
    // Atualizar PC se pontuação perfeita
    if (perfectScore) {
...
function startTimer(category) {
    ...
    if (isQuestAvailable(category)) return;
    ...
    const updateTimer = () => {
        ...
        setTimeout(updateTimer, 60000); // Atualizar a cada minuto
    };
